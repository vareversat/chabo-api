...
func ComputeForecasts(forecasts *[]models.Forecast, openDataForecasts models.OpenDataAPIResponse) {
	...
	for _, openAPIForecast := range openDataForecasts.Records {
		...
		circulationClosingDate := FormatDataTime(openAPIForecast.Fields.ClosingTime, openAPIForecast.Fields.ClosingDate, offset, *time.UTC)
		circulationReopeningDate := FormatDataTime(openAPIForecast.Fields.OpeningTime, openAPIForecast.Fields.ClosingDate, offset, *time.UTC)
		...
		*forecasts = append(*forecasts, models.Forecast{
			...
			Boats:                    MapBoats(closingReason, openAPIForecast.Fields.Boat, closingDuration, circulationClosingDate, &alreadySeenBoatNames, openAPIForecast.RecordID),
			Link:                     models.OpenAPISelfLink{Self: models.OpenAPILink{Link: "/v1/forecasts/" + openAPIForecast.RecordID}},
		})
	}
